...
use crate::types::{Order, OrderStatus, OrderType, Side, Symbol, TfTrade, Trade, TradeEntry};
...
impl<'a> OrderStateMachine<'a> {
    ...
    pub async fn process_order(&mut self, trade: &TfTrade) {
    ...
    async fn process_market_order(&mut self, trade: &TfTrade) {
    ...
    async fn process_limit_order(&mut self, trade: &TfTrade) {
        if !self.intersects(trade) {
            return;
        }
        let mut lock = self.positions.lock().await;
        let position = lock.get_mut(&self.order.symbol).unwrap();
    ...
    async fn process_stop_loss_limit_order(&mut self, trade: &TfTrade) {
        let mut lock = self.positions.lock().await;
    ...
    pub fn intersects(&self, trade: &TfTrade) -> bool {
        self.order
            .price
            .checked_sub(trade.low)
            .unwrap_or(dec!(1))
            .checked_mul(self.order.price.checked_sub(trade.high).unwrap_or(dec!(1)))
            .unwrap_or(dec!(1))
            .le(&dec!(0.0))
    }
}
